...
(defun kel-split-string (str)
  ...
  (let ((output nil)
        (current-str str))
    (while (not (equal current-str ""))
      (message (format "current-string: '%s'" current-str))
      (pcase (substring current-str 0 1)
        (" " (while (and (not (equal current-str "")) (equal (substring current-str 0 1) " "))
               (message "consuming whitespace")
               (setq current-str (substring current-str 1))))
        ("'" (let ((index 1))
               (while (and (not (equal current-str "")) (equal (substring current-str (- index 1) index) "'") (< index (length current-str)))
                 (message "consuming quoted")
                 (setq index (+ index 1)))
               (when (equal (substring current-str (- index 1) index) " ")
                 (setq index (- index 1)))
               (setq output (cons (substring current-str 0 index) output))
               (setq current-str (substring current-str index))))
        ("\"" (let ((index 1))
               (while (and (not (equal current-str "")) (equal (substring current-str (- index 1) index) "\"") (< index (length current-str)))
                 (message "consuming double quoted")
                 (setq index (+ index 1)))
             (when (equal (substring current-str (- index 1) index) " ")
               (setq index (- index 1)))
             (setq output (cons (substring current-str 0 index) output))
        ...
        (_ (let ((index 1)
                 (escaped nil))
             (message "catch all")
             (while (and (not (equal current-str "")) (or escaped (not (equal (substring current-str (- index 1) index) " "))) (< index (length current-str)))
               (message "catch all")
               (if (equal (substring current-str (- index 1) index) "\\")
                   (setq escaped t)
                 (setq escaped nil))
               (setq index (+ index 1)))
             (when (equal (substring current-str (- index 1) index) " ")
             ...
             (setq output (cons (substring current-str 0 index) output))
             (setq current-str (substring current-str index))))))
    (message (format "kel-split-string: %s %s" output (length output)))
    (reverse output)))
